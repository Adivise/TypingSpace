...
module.exports = { 
    name: ["leaderboard", "win"],
    description: "View the top win with leaderboard.",
    options: [
        {
            name: "page",
    ...
    run: async (client, interaction) => {
        ...
        for (let i = 0; i < user.length; i++) {
            ...
            userStrings.push(
                `**${i + 1}.** ${client.users.cache.get(fetch.id)} \`${formatNumber(e.win.toString())} 👑 Wins\`
                `);
        }
        ...
        for (let i = 0; i < pagesNum; i++) {
            ...
            const embed = new EmbedBuilder()
                .setAuthor({ name: `Top Win`, iconURL: interaction.guild.iconURL({ dynamic: true }) })
