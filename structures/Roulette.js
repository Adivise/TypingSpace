...
const { formatNumber } = require("./FormatMoney.js");

const giveMoney_2 = async (guildId, player, amount) => {
    const db = await Member.findOne({ id: guildId, user: player });
    db.money += amount;
    await db.save();
...
const pushArray = async (guildId, player, amount, space) => {
    const db = await Roulette.findOne({ id: guildId });

    db.data.push(`<@${player}> **+${formatNumber(parseInt(amount).toString())}** | Place on: **${space}**`)
...
const revMoney = async (guildId, userId, money) => {
    const db = await Member.findOne({ id: guildId, user: userId });
